...
namespace Lab8_LeTruongMinhNhan
{
    class NhanVien
    {
        ...
        public void ThemNhanVien (string hoten, string ngaysinh, string diachi, string dienthoai, string index_bc)
        {
            string sql = string.Format("Insert Into NhanVien Values(N'{0}', '{1}', N'{2}', '{3}',{4})", 
                hoten, ngaysinh, diachi, dienthoai, index_bc);
        ...
        {
            string str = string.Format("Update NHANVIEN set HoTenNhanVien = N'{0}', NgaySinh = '{1}', diachi = N'{2}'," +
                "  dienthoai = '{3}', MaBangCap = {4} where MaNhanVien = {5}", hoten, ngaysinh, diachi, dienthoai, index_bc, index_nv);
